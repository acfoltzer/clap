...
fn required_unless_any_2() {
    // This tests that the required_unless_any works when the second arg in the array is used
...
fn required_unless_any_works_with_short() {
    ...
    let res = App::new("unlessone")
        ...
        .arg(Arg::new("x").short('x').required_unless_any(&["a", "b"]))
...
fn required_unless_any_works_without() {
    let res = App::new("unlessone")
        .arg(Arg::new("a").conflicts_with("b").short('a'))
        .arg(Arg::new("b").short('b'))
        .arg(Arg::new("x").required_unless_any(&["a", "b"]))
        .try_get_matches_from(vec!["unlessone", "-a"]);
...
fn requires_if_present_mult() {
    let res = App::new("unlessone")
        .arg(
            Arg::new("cfg")
                .requires_if_eq_any(&[("extra", "my.cfg"), ("other", "other.cfg")])
                .takes_value(true)
                .long("config"),
        )
        .arg(Arg::new("extra").long("extra"))
