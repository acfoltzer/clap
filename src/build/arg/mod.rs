...
impl<'help> Arg<'help> {
    ...
    /// present see [`Arg::required_unless_any`]
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    /// assert_eq!(res.unwrap_err().kind, ErrorKind::MissingRequiredArgument);
    /// ```
    /// [`Arg::required_unless_any`]: ./struct.Arg.html#method.required_unless_any
    ...
    /// ```
    ///
    /// Setting [`Arg::required_unless_any(names)`] requires that the argument be used at runtime
    ...
    /// let res = App::new("prog")
    ///     .arg(Arg::new("cfg")
    ///         .required_unless_any(&["dbg", "infile"])
    ///         .takes_value(true)
    ///         .long("config"))
    ///     .arg(Arg::new("dbg")
    ///         .long("debug"))
    ...
    /// ```
    ///
    /// Setting [`Arg::required_unless_any(names)`] and *not* supplying *at least one of* `names`
    ...
    /// [`Arg::required_unless_any(names)`]: ./struct.Arg.html#method.required_unless_any
    /// [`Arg::required_unless_all`]: ./struct.Arg.html#method.required_unless_all
    pub fn required_unless_any(mut self, names: &[&str]) -> Self {
        self.r_unless.extend(names.iter().map(Id::from));
        self
    }
    ...
    /// # use clap::Arg;
    /// Arg::new("config")
    ///     .requires_if_eq("arg", "val")
    /// # ;
    /// ```
    ///
    /// Setting [`Arg::requires_if_eq(arg, val)`] requires that the `arg` be used at runtime if the
    ...
    /// ```
    ///
    /// Setting [`Arg::requires_if_eq(arg, val)`] and setting the value to `val` but *not* supplying
    /// `arg` is an error.
    ///
    /// ```rust
    /// # use clap::{App, Arg, ErrorKind};
    ...
    fn from(y: &'a Yaml) -> Self {
        ...
        for (k, v) in arg_settings.iter() {
            a = match k.as_str().unwrap() {
                ...
                "required_unless_any" => yaml_vec_or_str!(v, a, required_unless),
                "required_unless_all" => {
